...
def pedir_entero(entrada: str) -> int:
    entero: bool = False
    while not entero:
        try:
            valor: int = int(input(entrada))
            return valor
        except ValueError:
            print('Error: Debe introducir un número entero válido')

    return 0 #Nunca se va a alcanzar, es para que Mypy no de error



def seleccionar_refugio(mis_refugios: list[Refugio]) -> Union[Refugio, None]: #Para siempre que haya que seleccionar un refugio y mostrar todos sus nombres
    ...
    refugio_seleccionado: Union[Refugio, None] = None

    while refugio_seleccionado is None:
        refu: str = input('¿Que refugio elijes?: ')
        for refugio in mis_refugios:
            if refugio.nombre.lower() == refu.lower():
                refugio_seleccionado = refugio

        if refugio_seleccionado is None:
...
def anyadir_animal(mis_refugios: list[Refugio]) -> None: #Elección 2 en la función animales
    ...
    if seleccion == 1:  # Añadir perro
        ...
        refugio_seleccionado: Refugio | None = seleccionar_refugio(mis_refugios)
        if refugio_seleccionado is None:
            return
        ...
        if len(refugio_seleccionado.animales) >= refugio_seleccionado.tamanyo:
            ...
            edad: int = pedir_entero('Edad: ')
            raza: str = input('Raza: ')

            nuevo_perro: Perro = Perro(nombre, edad, raza, refugio_seleccionado)
        ...
        if len(refugio_seleccionado.animales) >= refugio_seleccionado.tamanyo:
            ...
            nuevo_gato: Gato = Gato(nombre, edad, raza, refugio_seleccionado)
        ...
        if len(refugio_seleccionado.animales) >= refugio_seleccionado.tamanyo:
            ...
            nuevo_caballo: Caballo = Caballo(nombre, edad, raza, refugio_seleccionado)
            print('Se ha introducido correctamente al caballo en la base de datos\n')



def animales(mis_refugios: list[Refugio], lista_usuarios: list[Usuario]) -> None:
    ...
    while seguir:
        ...
        seleccion: int = pedir_entero('Elija un apartado (1/2/3/4/5): ')
        print()
        ...
        if seleccion == 1:  # Añadir animal
            ...
            if len(refugio_seleccionado.animales) == 0:
                ...
                elegido: bool = False
                while not elegido:
                    seleccion_codigo: str = input('Seleccione el código de un animal: ').upper()
                    for animal in refugio_seleccionado.animales:
                        if seleccion_codigo == animal.codigo:
                            elegido = True
                            nombre_final: Animales = animal
            ...
            if len(refugio_seleccionado.animales) == 0:
                ...
                comida: str = input('¿Que comida debe tomar el animal? ')

                if nombre_final.especie == 'gato':
                    estado_garras: str = input('¿En que estado se encuentran las garras del gato?: ')
                    enfermo: str = input('¿El gato está enfermo? (s/n) ').lower()
                    if enfermo == 's':
                        enfermedad: str = input('¿Que enfermedad padece? ')
                        cura: str = input('¿Cual es la cura de la enfermedad? ')
                        informe: str = nombre_final.inspeccion(comida, estado_garras, enfermedad, cura)
                    else:
                        informe: str = nombre_final.inspeccion(comida, estado_garras)


                elif nombre_final.especie == 'perro':
                    estado_dientes: str = input('¿En que estado se encuentran los dientes del perro?: ')
                    enfermo: str = input('¿El perro está enfermo? (s/n) ').lower()
                    if enfermo == 's':
                        enfermedad: str = input('¿Que enfermedad padece? ')
                        cura: str = input('¿Cual es la cura de la enfermedad? ')
                        informe: str = nombre_final.inspeccion(comida, estado_dientes, enfermedad, cura)
                    else:
                        informe: str = nombre_final.inspeccion(comida, estado_dientes)


                elif nombre_final.especie == 'caballo':
                    estado_pezunyas: str = input('¿En que estado se encuentran las pezuñas del caballo?: ')
                    enfermo: str = input('¿El caballo está enfermo? (s/n) ').lower()
                    if enfermo == 's':
                        enfermedad: str = input('¿Que enfermedad padece? ')
                        cura: str = input('¿Cual es la cura de la enfermedad? ')
                        informe: str = nombre_final.inspeccion(comida, estado_pezunyas, enfermedad, cura)
                    else:
                        informe: str = nombre_final.inspeccion(comida, estado_pezunyas)
                ...
                nombre_archivo: str = f"{nombre_carpeta}/Informe_{nombre_final.codigo}_{nombre_final.nombre}.txt"
            ...
            dni: str = pedir_dni()
            cliente: Usuario | None = buscar_usuario(dni, lista_usuarios)
            ...
            if len(refugio_seleccionado.animales) == 0:
                ...
                refugio_seleccionado.mostrar_animales()
                codigo: str = input('Introduce el código del animal a adoptar: ')

                animal_adoptado: str = refugio_seleccionado.adoptar(codigo)

                if animal_adoptado:
                    mensaje_exito: str = cliente.registrar_adopcion(animal_adoptado)
                    print(mensaje_exito)
                    carpeta_historial: str = 'Historial'

                    if not os.path.exists(carpeta_historial):
                        os.makedirs(carpeta_historial)

                    try:
                        ruta_txt: str = os.path.join(carpeta_historial, 'Historial_Adopción.txt')
                        fecha: str = datetime.now().strftime('%d/%m/%Y %H:%M:%S')
...
def refugios(mis_refugios: list[Refugio]) -> list[Refugio]:
    seguir: bool = True
    while seguir:
        ...
        seleccion: int = pedir_entero('Elija un apartado (1/2/3): ')
        print()
        ...
        if seleccion == 1:# Mirar los refugios que tiene
            ...
            nombre: str = input('Nombre del refugio: ').lower()
            tamanyo: int = pedir_entero('Espacio máximo del refugio: ')
            print()

            mi_refugio: Refugio = Refugio(nombre, tamanyo)
...
def anyadir_empleado(mis_refugios: list[Refugio]) -> None: #
    seleccion: int = pedir_entero('¿Cuál es su futura ocupación? ')
    ...
    if seleccion == 1:  #Añadirlo como cuidador
        ...
        nombre: str = input('Nombre: ')
        edad: int = pedir_entero('Edad: ')
        genero: str = input('Género(F/M): ').upper()
        dni: str | None = pedir_dni()
        ...
        genero: str = input('Género(F/M): ').upper()
        dni: str | None = pedir_dni()
        salario: int = pedir_entero('Introduzca el salario inicial acordado: ')
...
def trabajadores(mis_refugios: list[Refugio]) -> None:
    seguir: bool = True
    while seguir:
        ...
        if seleccion == 1:  # Añadir empleado
            ...
            if len(refugio_seleccionado.empleados) == 0:
                ...
                if nombre_final.oficio == 'Cuidador':
                    animal: str = input('¿Con que animal ha estado trabajando? ')
                    horas: int = pedir_entero('¿Cuantas horas ha trabajado? ')
            ...
            if len(refugio_seleccionado.empleados) == 0:
                ...
                if nombre_final.oficio == 'Cuidador':
                    ...
                    input("si está usted de acuerdo con los téminos y condiciones pulse [ENTER]")
...
def buscar_usuario(dni: str, lista_usuarios: list[Usuario]) -> Usuario | None:
...
def clientes(mis_refugios: list[Refugio], lista_usuarios: list[Usuario]) -> list[Usuario] | None:
    seguir: bool = True
    while seguir:
        ...
        seleccion: int = pedir_entero('Elija un apartado (1/2/3/4/5/6): ')
        print()
        ...
        if seleccion == 1:  # Añadir Usuario
            ...
            if buscar_usuario(dni, lista_usuarios):
                ...
                contacto: int = pedir_entero('Número de contacto: ')
                genero: str = input('Género(F/M): ').upper()
                residencia: str = input('Dirección: ')
                nuevo_u: Usuario = Usuario(nombre, dni, contacto, residencia, genero)
            ...
            if cliente:
                ...
                cuota: int = pedir_entero("Ingrese cuota mensual: ")
                
                # Convertimos o creamos el socio
                nuevo_socio: Socio = Socio(cliente.nombre, cliente.dni, cliente.contacto, cliente.residencia, cliente.genero, cuota)
            ...
            if cliente:
                edad: int = pedir_entero("Edad: ")
                refugio_asig: Refugio | None = seleccionar_refugio(mis_refugios)
                if refugio_asig is None:
                    return
                nuevo_vol: Voluntario = Voluntario(cliente.nombre, edad, cliente.genero, cliente.dni, cliente.contacto, cliente.residencia, 0, refugio_asig)
                lista_usuarios.remove(cliente)
            ...
            if isinstance(cliente, Voluntario):
                horas: int = pedir_entero("¿Cuántas horas ha trabajado hoy? ")
                tarea: str = input("Tarea realizada: ")
            ...
            cliente: Usuario | None = buscar_usuario(dni, lista_usuarios)
            if cliente:
                nuevo_c: int = pedir_entero("Nuevo contacto: ")
...
def guardar_datos(mis_refugios: list[Refugio], lista_usuarios: list[Usuario]) -> None:
    carpeta_base_datos: str = 'BaseDatos'

    if not os.path.exists(carpeta_base_datos):
        os.makedirs(carpeta_base_datos)

    ruta_pkl: str = os.path.join(carpeta_base_datos, 'datos_sistema.pkl')
    try:
...
def cargar_datos() -> tuple[list[Refugio], list[Usuario]]:
    ruta_pkl: str = os.path.join('BaseDatos', 'datos_sistema.pkl')
